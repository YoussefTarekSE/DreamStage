...
import { BeatFlow } from "@/components/studio/BeatFlow";
...
export default async function BeatPage({
  ...
  const { data: project } = await supabase
    .from("projects")
    .select("id, name, beat_key, beat_attempts, status")
    .eq("id", projectId)
    .eq("user_id", user.id)
    .maybeSingle();
  ...
  return (
    <div className="min-h-screen bg-zinc-950 flex flex-col">
        ...
        <div className="mx-auto max-w-lg flex items-center gap-2 text-xs text-zinc-500">
          {["Vocal", "Beat", "Coach", "Mix"].map((s, i) => (
            <div key={s} className="flex items-center gap-2">
              ...
              <span className={i === 1 ? "text-violet-400 font-medium" : ""}>{s}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <main className="flex flex-1 flex-col items-center justify-center px-6 py-12">
        <BeatFlow
          projectId={projectId}
          hasExistingBeat={Boolean(project.beat_key)}
          existingAttempts={project.beat_attempts ?? 0}
        />
      </main>
    </div>
  );
